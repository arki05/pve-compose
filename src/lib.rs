...
//! * `config`: `/etc/pve/pve-compose.cfg`, the operator's own settings.
...
/// Where the stack lives inside the guest. Fixed: it is the one path the
/// operator, the rendered compose file and a human at the shell all agree on.
